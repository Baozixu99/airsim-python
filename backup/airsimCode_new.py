...
server_address = ("10.30.162.7", PORTTOSEND)  # 该ip接收方 服务器的ip地址和端口号
server_address2 = ("10.30.162.7", PORTTOSEND2)  # 该ip接收方 服务器的ip地址和端口号
...
def th3():
    ...
    client.moveToPositionAsync(48.29, -50.56, -28.01, 5, vehicle_name="UAV3").join()
    for i in range(3):  # 锁定
        name = "UAV" + str(i + 1)
        if i == 0:
...
def th4():
    ...
    while True:
        ...
        responses = client3.simGetImages([
            ...
            vehicle_name="UAV1")
        # file1 = open("images/" + "UAV1" + "/" + str(i) + "uav1.png", 'wb') #为了vscode展示，以固定命名覆盖
        file1 = open("images/" + "UAV1" + "/bottom1.png", 'wb')  # 为了vscode展示，以固定命名覆盖
...
def th5():
    ...
    while True:
        ...
        responses = client4.simGetImages([
            airsim.ImageRequest("bottom_center", 0, pixels_as_float=False, compress=True)],
            vehicle_name="UAV2")
        # file1 = open("images/" + "UAV2" + "/" + str(i) + "uav2.png", 'wb')  # 为了vscode展示，以固定命名覆盖
...
def th6():
    ...
    while True:
        ...
        responses = client5.simGetImages([
            airsim.ImageRequest("bottom_center", 0, pixels_as_float=False, compress=True)],
            vehicle_name="UAV3")
        # file1 = open("images/" + "UAV3" + "/" + str(i) + "uav3.png", 'wb')  # 为了vscode展示，以固定命名覆盖
...
def th7():
    ...
    totaldisplacement = np.array([0.0,0.0,0.0])
    lastdisplacementx = np.array([0.0,0.0,0.0])
    lastdisplacementy = np.array([0.0,0.0,0.0])
    while True:
        uav_state1 = client6.getMultirotorState(vehicle_name="UAV1").kinematics_estimated
        uav_state2 = client6.getMultirotorState(vehicle_name="UAV2").kinematics_estimated
        uav_state3 = client6.getMultirotorState(vehicle_name="UAV3").kinematics_estimated
        tmpdisplacementx = np.array([uav_state1.position.x_val, uav_state2.position.x_val,uav_state3.position.x_val])
        tmpdisplacementy = np.array([uav_state1.position.y_val, uav_state2.position.y_val,uav_state3.position.y_val])
        totaldisplacement += np.sqrt(np.power(tmpdisplacementx-lastdisplacementx,2)+np.power(tmpdisplacementy-lastdisplacementy,2))
        lastdisplacementx = tmpdisplacementx
        lastdisplacementy = tmpdisplacementy
        print("位移："+str(totaldisplacement))
        time.sleep(5)
