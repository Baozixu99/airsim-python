...
# Make sure to place energy_efficient_sensors.py in the same directory.
from energy_efficient_sensors import select_optimal_sensors
# 声明全局变量，便于长按监听
current_key = None
# 声明全局默认飞行速度
default_speed = 5
...
def th3():
    ...
    for i in range(3):
        # Example task requirements and context
        task_requirements = {"data_type": "visual", "resolution": "high"}
        context = {"area": "urban", "time_of_day": "daytime"}

        # Select the optimal sensor configuration
        optimal_sensor_config = select_optimal_sensors(task_requirements, context)
        print("Optimal sensor configuration:", optimal_sensor_config)

        # Placeholder for applying the optimal sensor configuration
        # This should be replaced with actual API calls based on optimal_sensor_config
        # Example: configure_uav_sensors(uav_name, optimal_sensor_config)
...
def th4():
    while True:
        ...
        responses = client3.simGetImages([
            airsim.ImageRequest("bottom_center", type1, pixels_as_float=False, compress=True)],
            vehicle_name="UAV1")
        file1 = open("images/" + "UAV1" + "/" + "bottom" + ".png", 'wb') #为了vscode展示，以固定命名覆盖
